...
public class PurchaseOrderService {
    ...
    public List<PurchaseOrder> findOrdersByUserId(long userId) {
        List<PurchaseOrderCreationDTO> orders = purchaseRepository.findByUserId(userId);
        if (orders.isEmpty()) {
            throw new OrderNotFoundException("No orders found for User id: " + userId);
        }
        return (List<PurchaseOrder>) purchaseMapper.toPurchaseOrderResponseDTO((PurchaseOrder) orders);
    }
